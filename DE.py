...
def de(m=8, n=8):
    ...
    g = 1  # 初始化代数：第一代
    while g <= GMAX:        # 退出条件1:达到最大迭代次数；条件2:最优值不再变化(此条件不成立)
        print('------------------------------------------------')
        ...
        # ------选择------
        for i in range(NP):
            if gmax_func(gnext_2[i, :]) >= gmax_func(g0[i, :]):
                gnext[i, :] = gnext_2[i, :]
            else:
                gnext[i, :] = g0[i, :]
...
def ps_gg_theo(a):
    """
    给定分布函数a，获取失败概率q值的迭代变化
    :param a:分布函数a
    :return:失败概率q值的迭代变化
    """
    q = []
    for gg in np.arange(0, 1, 0.001):
        result_bool, q_last = q_iterative(a, gg)
        p_suc = 1 - q_last
        q.append(p_suc)
    # filename = "ps_gg_theo_n=" + str(len(a)-1)
    # DS.store2file(q, filename, 0)
    return q


def get_q_all():
    """
    获取并保存所有天线数对应的迭代收敛概率q
    :return:q_all 字典形式{4:[1, 0.5, 0.2, 0.1, 0]}
    """
    a_dict = configuration.a_dict   # a_dict中使用字典形式保存了所有的分布
    q_all = {}  # 保存所有天线数对应的迭代收敛概率q
    for key in a_dict:
        print("Computing for q_all of n = {}".format(key))
        a_temp = a_dict[key]
        q = DE.ps_gg_theo(a_temp)
        q_all[key] = q
    DS.store2file(q_all, "q_all", 0)


def get_gstar(a):
    """
    已知分布序列，获取其对应的G*值, 向下取整取小数
    :return:
    """
    # a = []
    gstar = gmax_func(a)
    print("分布为：a = {}".format(a))
    print("对应的G*为：gstar = {}".format(gstar))


if __name__ == "__main__":
    a = configuration.a_dict[32]
    res = 0
    show = ''
    ii = 0
    for i in a:
        ...
        res = res + i
        if i is not 0:
            show = show + '{}x^{}+'.format(i, ii)
        ii = ii + 1
    print("show = {}".format(show))
    print("sum_a = {}".format(res))
    get_gstar(a)
